		...
		if (mop != null) {
			...
			if (mop.typeOfHit == EnumMovingObjectType.TILE) {
				if (par1ItemStack.getItemDamage() == 1 || par1ItemStack.getItemDamage() == 3) {
					...
					//Pickup Oil
					else if(world.getBlockId(X, Y, Z) == BuildCraftEnergy.oilStill.blockID && par1ItemStack.getItemDamage() == 3) {
					...
					//Pickup Water
					if((world.getBlockId(X, Y, Z) == Block.waterStill.blockID || world.getBlockId(X, Y, Z) == TFCBlocks.finiteWater.blockID)
				...
				if (par1ItemStack.getItemDamage() == 2 || par1ItemStack.getItemDamage() == 4) {
					...
					if (world.isAirBlock(X, Y, Z) || !world.getBlockMaterial(X, Y, Z).isSolid()) {
						...
						//Put Water
							...
							world.setBlock(X, Y, Z, TFCBlocks.finiteWater.blockID);
							if (par3EntityPlayer.capabilities.isCreativeMode) {
								return par1ItemStack;
							}
							return new ItemStack(Items.Buckets, 1, 1);
						...
						//Put Oil
							...
							world.setBlock(X, Y, Z, BuildCraftEnergy.oilMoving.blockID);
							if (par3EntityPlayer.capabilities.isCreativeMode) {
								return par1ItemStack;
							}
							return new ItemStack(Items.Buckets, 1, 3);
