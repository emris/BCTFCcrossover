...
public class WorldGenOil implements IWorldGenerator
{
	...
	public void generate(Random random, int chunkX, int chunkZ, World world, IChunkProvider chunkGenerator, IChunkProvider chunkProvider)
	{
		...
		{
			int oilRockLayers = TFCBlocks.StoneSed.blockID; /*spawn only in sedimentary stone*/
			...
			if(rockLayer1.data1 == oilRockLayers)
			{
	...
	private static void genShallowOil(World world, Random rand, int chunkX, int chunkZ, DataLayer rockLayer2)
	{
		...
		{
			if(world.getBlockId(chunkX, l, chunkZ) == rockLayer2.data1 && world.getBlockMetadata(chunkX, l, chunkZ) == rockLayer2.data2)
			{
				chunkY = l;
				break;
			}
	...
	private static void genDeepOil(World world, Random rand, int chunkX, int chunkZ, DataLayer rockLayer3)
	{
		...
		{
			if(world.getBlockId(chunkX, l, chunkZ) == rockLayer3.data1 && world.getBlockMetadata(chunkX, l, chunkZ) == rockLayer3.data2)
			{
				chunkY = l;
				break;
			}
			...
			for(int zCoord = chunkZ - radius; zCoord <= chunkZ + radius; zCoord++)
			{
				...
				if (x * x + z * z <= radius * radius)
				{
					for(int yCoord = chunkY - height; yCoord <= chunkY; yCoord++)
					{
						...
						{
							world.setBlock(xCoord, yCoord, zCoord, BuildCraftEnergy.blockOil.blockID, 0, 0x2);
	...
	private static void genTunnel(World world, Random rand, int chunkX, int chunkZ, int chunkY, int radius)
	{
		...
		while(makeTunnel)
		{
			if(world.getBlockId(xCoord, yCoord + 1, zCoord) == 0) makeTunnel = false;
